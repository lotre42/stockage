...
static void xorid(t_stock *info, int pc, int *tabtype, int *tabvalue)
{
	if (tabtype[1] == 3 && tabtype[2] == 3)
	{
		if ((info->ram[pc + (tabvalue[0] % 512)] && !info->ram[pc + (tabvalue[1] % 512)])
		|| (info->ram[pc + (tabvalue[1] % 512)] && !info->ram[pc + (tabvalue[0] % 512)]))
	...
	else if (tabtype[1] == 3 && tabtype[2] == 1)
	{
		if ((info->ram[pc + (tabvalue[0] % 512)] && !info->registre[tabvalue[1]]) ||
		 (!info->ram[pc + (tabvalue[0] % 512)] && info->registre[tabvalue[1]]))
			info->registre[tabvalue[2]] = 1;
		else
			info->registre[tabvalue[2]] = 0;
	}
	else if (tabtype[1] == 3 && tabtype[2] == 2)
	{
		if ((tabvalue[1] && !info->ram[pc + (tabvalue[0] % 512)]) ||
		 (!tabvalue[1] || info->ram[pc + (tabvalue[0] % 512)]))
			info->registre[tabvalue[2]] = 1;
		else
			info->registre[tabvalue[2]] = 0;
	}
...
static void xorr(t_stock *info, int pc, int *tabtype, int *tabvalue)
{
	...
	else if (tabtype[1] == 1 && tabtype[2] == 3)
	{
		...
		(!info->registre[tabvalue[0]] && info->ram[pc + (tabvalue[1] % 512)]))
			info->registre[tabvalue[2]] = 1;
		else
			info->registre[tabvalue[2]] = 0;
	}
...
static void xordi(t_stock *info, int pc, int *tabtype, int *tabvalue)
{	
	...
	else if (tabtype[1] == 2 && tabtype[2] == 3)
	{
		...
		(!tabvalue[0] && info->ram[pc + (tabvalue[1] % 512)]))
			info->registre[tabvalue[2]] = 1;
		else
			info->registre[tabvalue[2]] = 0;
	}
	else
		xorr(info, pc, tabtype, tabvalue);	
}
