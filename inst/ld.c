...
void	ld(t_stock *info)
{
	...
	if (tabtype[1] == 3)
	{
		info->registre[tabvalue[1]] = (info->ram[pluspc(info->pc, (tabvalue[0] % 512))] << 24)
		   	| ((info->ram[pluspc(info->pc, 1 + (tabvalue[0] % 512))] << 16) & 0x00ffffff)
		   	| ((info->ram[pluspc(info->pc, 2 + (tabvalue[0] % 512))] << 8) & 0x0000ffff)
